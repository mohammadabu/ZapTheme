...
class HrLeaveTypesCarryForwards(models.Model):
    ...
    def carryForwordsDaily(self):     
        _logger.info("---------------------------")
        annual_leave_type = self.env['hr.leave.type'].sudo().search([('carry_forwards','=','True'),('finished_carry_froword','=',False),'&',('validity_stop','>',datetime.strptime(str(datetime.today()),'%Y-%m-%d'))])
